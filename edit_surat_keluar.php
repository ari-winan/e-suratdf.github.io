<?php 
		if (isset($_REQUEST['edit'])) {
			...
			//jika mengubah file
				$query = "UPDATE surat_keluar SET no_agenda='$no_agenda', no_surat='$no_surat', jenis_surat='$jenis_surat', tgl_kirim='$tanggal_kirim', tujuan='$tujuan', perihal='$isi_ringkas', file='$file' WHERE id='$_GET[id]'";
				$sql = mysqli_query($connect, $query);
				if ($sql) {
					echo '<script> 
			...
			//jika tidak mengubah file
				$query ="UPDATE surat_keluar SET no_agenda='$no_agenda', no_surat='$no_surat', jenis_surat='$jenis_surat', tgl_kirim='$tanggal_kirim', tujuan='$tujuan', perihal='$isi_ringkas' WHERE id='$_GET[id]'";
				$sql = mysqli_query($connect, $query);
				if($sql){
					echo '<script>
							window.alert("Data berhasil di ubah");
					...
					<div class="x_content">
						...
						<form class="form-horizontal form-label-left" method="post" enctype="multipart/form-data">
							...
							<div class="item form-group">
								...
								<div class="col-md-6 col-sm-6 col-xs-12">
									<input type="text" name="tgl_kirim" class="form-control has-feedback-left" id="tanggal" required="required" value="<?php echo IndonesiaTgl($data['tgl_kirim']);?> ">
